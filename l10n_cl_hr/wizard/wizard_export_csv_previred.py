...
class WizardExportCsvPrevired(models.TransientModel):
    ...
    def get_cost_center(self, contract):
        cost_center = "0"
        if contract.analytic_account_id and contract.analytic_account_id.code and contract.analytic_account_id.code.isnumeric():
    ...
    @api.model
    def get_imponible_ccaf(self, payslip, TOTIM):
    ...
    def get_imponible_afp(self, payslip, TOTIM):
        if payslip.contract_id.pension is True and payslip.contract_id.pension_afp is False:
            return '0'
        ...
        elif TOTIM in [False, 0, '00', '0']:
            return int(payslip.contract_id.wage)
        else:
    ...
    @api.model
    def get_imponible_ips(self, payslip, TOTIM):
        if TOTIM >= round(payslip.indicadores_id.tope_imponible_ips*payslip.indicadores_id.uf_2):
            return int(round(payslip.indicadores_id.tope_imponible_ips*payslip.indicadores_id.uf_2))
        else:
            return int(round(TOTIM))
    ...
    def action_generate_csv(self):
        ...
        country_company = self.env.company.country_id
        output = io.StringIO()
        ...
        payslip_recs = payslip_model.search([('date_from','=',self.date_from),('company_id' ,'=',self.env.company.id)
                                             ])
        ...
        try:
            rut_emp, rut_emp_dv = self.env.company.vat.split("-")
        ...
        for payslip in payslip_recs:
            ...
            #00 Linea Principal o Base
            #01 Linea Adicional
            # Por ahora colocaremos todos los movimientos
            linea = '00'
            temp = 0
            movimientos = len(payslip.worked_days_line_ids.filtered(lambda x: x.movimientos_personal not in ['0','Sin Movimiento en el Mes',False])) or 0
            #Solo tomamos en cuenta WORK100 o licencias
            for mov in payslip.worked_days_line_ids.filtered(lambda x: x.movimientos_personal not in ['0','Sin Movimiento en el Mes'] or x.code=='WORK100'):
                #Si hay mas de un movimiento de personal y estamos en la linea base saltar

                if (movimientos > 0 and mov.code=='WORK100'):
                    continue
                ...
                line_employee = [self._acortar_str(rut, 11),
                                 ...
                                 #13
                                 "0" if temp > 0 else (int(self.get_dias_trabajados(payslip and payslip[0] or False)) + int(payslip[0].vacaciones)),
                                 #14
                                 "01" if temp > 0 else "00",
                                 #15
                                 mov.movimientos_personal or '0',
                                 ...
                                 str(mov.date_from.strftime("%d/%m/%Y")) if mov.movimientos_personal != '0' and mov.date_from else '00/00/0000',
                                 #payslip.date_from if payslip.date_from else '00/00/0000',
                                 #17 Fecha fin movimiento personal (dia-mes-año)
                                 str(mov.date_to.strftime("%d/%m/%Y")) if mov.movimientos_personal != '0' and mov.date_to else '00/00/0000',
                                 #Si declara mov. personal 1, 3, 4, 5, 6, 7, 8 y 11 Fecha Desde
                                 #es obligatoria y debe estar dentro del periodo de remun
                                 ...
                                 "0",
                                 # 25 Solicitud Trabajador Joven TO-DO SUBSIDIO JOVEN
                                 "N",
                                 #26
                                 ...
                                 "0",
                                 # 46 Numero de Contrato APVC TO-DO
                                 " ",
                                 ...
                                 int(self.get_imponible_afp(payslip and payslip[0] or False, self.get_payslip_lines_value_2(payslip,'TOTIM'))),
                                 #int(self.get_imponible_ips(payslip and payslip[0] or False, self.get_payslip_lines_value_2(payslip,'TOTIM'))) if self.get_payslip_lines_value_2(payslip,'TOTIM') else "0",
    ...
    #caso se paga en la mutual respectiva. Datos no numérico
                                 ...
                                 #78 Moneda Plan Isapre UF Pesos TO-DO Poner % Pesos o UF
                                 ...
                                 #83 Codigo CCAF
                                 # TO-DO ES HACER PANTALLA CON DATOS EMPRESA
                                 payslip.indicadores_id.ccaf_id.codigo if payslip.indicadores_id.ccaf_id.codigo else "00",
                                 #84 Renta Imponible CCAF
                                 int(self.get_imponible_ccaf(payslip and payslip[0] or False, self.get_payslip_lines_value_2(payslip,'TOTIM'))) if (self.get_dias_trabajados(payslip and payslip[0] or False)>0 and payslip.indicadores_id.ccaf_id and payslip.indicadores_id.ccaf_id.codigo != '00') else "0",
                                 #85 Creditos Personales CCAF TO-DO
                                 ...
                                 "0",
                                 #87 Descuentos por Leasing TO-DO
                                 "0",
                                 #88 Descuentos por seguro de vida TO-DO
                                 "0",
                                 ...
                                 #97 Renta Imponible Mutual TO-DO Si afiliado hacer
                                 self.get_imponible_mutual(payslip and payslip[0] or False, self.get_payslip_lines_value_2(payslip,'TOTIM')) if self.get_payslip_lines_value_2(payslip,'MUT') else "0",
            ...
            self.write({'file_data': base64.encodebytes(output.getvalue().encode()),
                        'file_name': "Previred_%s_%s.txt" % (self.date_to,str(datetime.now()).replace('.','')),
                        })
