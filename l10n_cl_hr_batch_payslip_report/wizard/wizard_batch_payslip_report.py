...
class wizard_batch_payslip_report(models.TransientModel):
    ...
    def print_report_xls(self):
        ...
        worksheet.merge_range(row, 1, row + 1, 1, 'Liquidación', font_bold_center)
        worksheet.merge_range(row, 2, row + 1, 2, 'Empleado', font_bold_center)
        worksheet.merge_range(row, 3, row + 1, 3, 'RUT', font_bold_center)
        worksheet.merge_range(row, 4, row + 1, 4, 'Departamento', font_bold_center)
        ...
        for col_rule_id in col_lst:
            ...
            col += 1
        workbook.close()
        action = self.env.ref('l10n_cl_hr_batch_payslip_report.action_wizard_batch_payslip_report').read()[0]
    ...
    def print_report_pdf(self):
        data = self.read()[0]
        return self.env.ref('l10n_cl_hr_batch_payslip_report.action_print_batch_payslip').report_action([], data=data)
...
class l10n_cl_hr_batch_payslip_report_report_batch_payslip_template(models.AbstractModel):
    _name = 'report.l10n_cl_hr_batch_payslip_report_template'
    _description = 'Report Batch Payslip Report Template'

    @api.model
    def _get_report_values(self, docids, data=None):
        report = self.env['ir.actions.report']._get_report_from_name('l10n_cl_hr_batch_payslip_report.report_batch_payslip_template')
