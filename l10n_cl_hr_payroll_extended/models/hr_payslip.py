...
#                            ''' %(rec.id))
            ...
            for slip_rec in rec.slip_ids:
                payslip_data = {'employee_name': slip_rec.employee_id.name or '',
                                ...
                                'employee_department': slip_rec.employee_id.department_id.name or '',
                                'employee_job': slip_rec.contract_id.job_id.name or '',
            ...
            for payslip_data in all_payslip_data:
                if counter == 0:
                    ...
                    from_cell = xl_rowcol_to_cell(row, col)
                    to_cell = xl_rowcol_to_cell(row, col)
                    worksheet.write(str(from_cell) + ':' + str(to_cell), 'DEPARTAMENTO', header_rule_format_left)
                    col += 1
                    from_cell = xl_rowcol_to_cell(row, col)
                    to_cell = xl_rowcol_to_cell(row, col)
                    worksheet.write(str(from_cell) + ':' + str(to_cell), 'PUESTO DE TRABAJO', header_rule_format_left)
                ...
                from_cell = xl_rowcol_to_cell(row, col)
                to_cell = xl_rowcol_to_cell(row, col)
                worksheet.write(str(from_cell) + ':' + str(to_cell), payslip_data.get('employee_department'), emp_data_format)
                col += 1
                from_cell = xl_rowcol_to_cell(row, col)
                to_cell = xl_rowcol_to_cell(row, col)
                worksheet.write(str(from_cell) + ':' + str(to_cell), payslip_data.get('employee_job'), emp_data_format)
